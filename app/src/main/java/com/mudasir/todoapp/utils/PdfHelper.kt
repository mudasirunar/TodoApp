...
object PdfHelper {
    ...
    private fun drawSummaryTable(canvas: Canvas, y: Float, tasks: List<TodoTask>, config: PdfConfig) {
        ...
        if (config.includeStatus) {
            ...
            val sortedStatuses = listOf(com.mudasir.todoapp.data.TodoStatus.Done, com.mudasir.todoapp.data.TodoStatus.Ongoing, com.mudasir.todoapp.data.TodoStatus.ComingUp)
            sortedStatuses.forEach { status ->
                val count = tasks.count { it.status == status }
                val label = if (status == com.mudasir.todoapp.data.TodoStatus.ComingUp) "Pending" else status.label
